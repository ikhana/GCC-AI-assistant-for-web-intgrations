...
export function useRiveStateMachine({
  src,
  stateMachineName,
  autoplay = true,
}) {
  ...
  const successfulResponseInput = useStateMachineInput(
    rive,
    stateMachineName,
    "Response / successful"
  );
  const failResponseInput = useStateMachineInput(
    rive,
    stateMachineName,
    "Response / fail"
  );
  ...
  const transitionToThinking = useCallback(
    () => thinkingInput?.fire(),
    [thinkingInput]
  );
  ...
  const transitionToSuccessfulResponse = useCallback(
    () => successfulResponseInput?.fire(),
    [successfulResponseInput]
  );
  const transitionToFailResponse = useCallback(
